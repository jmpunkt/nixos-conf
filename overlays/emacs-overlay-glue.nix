...
in
{
  emacs-overlay = (super.emacs-overlay or { }) // {
    repos =
      let
        inherit (super)
          lib
    ...
    mkEmacsWithFeatures =
      features: builtins.foldl' (drv: fn: fn drv) emacs (features ++ [ forwardPkgs ]);
    ...
    enableDebug =
      ...
      drv.overrideAttrs (old: {
        ...
        configureFlags = old.configureFlags ++ (lib.singleton "--enable-check-lisp-object-type");
      });
  };
}
