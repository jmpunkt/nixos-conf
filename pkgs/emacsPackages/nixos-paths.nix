...
let
  valueToEmacs =
    value:
    if
      ( builtins.typeOf value ) == "list"
    then
      "'(${ builtins.concatStringsSep "\n" ( builtins.map valueToEmacs value ) })"
    else
      if
        ( builtins.isString value ) || ( builtins.isPath value )
      then
        ''"${ value }"''
      else
        if
          ( builtins.isNull value ) || ( ( builtins.isBool value ) && value == false )
        then
          "nil"
        else
          if
          ...
          then
            ...
            if
              ( builtins.isInt value ) || ( builtins.isFloat value )
            then
              "${ value }"
            else
