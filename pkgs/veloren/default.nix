{ stdenv, lib, mozillaNightlyRustPlatform, fetchurl,
  pkg-config,
  git, git-lfs, python3,
  alsaLib, atk, cairo, glib, gtk3, pango,
  server ? false,
  chat ? false,
}:
...
mozillaNightlyRustPlatform.buildRustPackage rec {
  ...
  src = fetchurl {
    url = "https://gitlab.com/veloren/veloren/-/archive/v0.5.0/veloren-${version}.tar.gz";
  ...
  cargoBuildFlags =
    (lib.optional (!server) "--bin veloren-voxygen")
